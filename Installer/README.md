...
```powershell
msiexec /i AIUsage-1.0.4-win-x64.msi REMOVE=AutoStart
```
...
```powershell
msiexec /x AIUsage-1.0.4-win-x64.msi
```
...
./build-msi.ps1                 # -> Installer/AIUsage-1.0.4-win-x64.msi
...
```powershell
dotnet tool restore
dotnet wix msi validate -sice ICE38 -sice ICE64 -sice ICE91 .\AIUsage-1.0.4-win-x64.msi
```
